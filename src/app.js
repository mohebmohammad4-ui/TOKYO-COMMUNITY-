...
class TitanBot extends Client { 
          ...
          await channel.send({ 
            embeds: [ 
              { 
                ...
                image: { url: 'https://i.ibb.co/C2fhyL6/1000290040.jpg' }, 
                footer: { text: 'Tokyo Community' }, 
                timestamp: new Date() 
              } 
            ], 
            components: [ 
              { 
                type: 1, 
                components: [ 
                  { type: 2, label: 'من نحن', style: 2, custom_id: 'about', emoji: { name: '🌸' } }, 
                  { type: 2, label: 'القوانين', style: 2, custom_id: 'rules', emoji: { name: '📜' } } 
                ] 
              }, 
              { 
                type: 1, 
                components: [ 
                  { type: 2, label: 'البوست', style: 2, custom_id: 'boost', emoji: { name: '💎' } }, 
                  { type: 2, label: 'الرتب', style: 2, custom_id: 'roles', emoji: { name: '🎌' } } 
                ] 
              } 
            ] 
          }); 
  ...
  startWebServer() { 
    ...
    app.get('/api/bot-commands', (req, res) => { 
      const commandList = this.commands.map(cmd => ({ name: cmd.name, description: cmd.description || 'لا يوجد وصف متاح' })); 
    ...
    app.get('/health', (req, res) => { 
      ...
      const status = { status: 'healthy', timestamp: new Date().toISOString(), uptime: process.uptime(), database: { connected: dbStatus.connectionType !== 'none', degraded: dbStatus.isDegraded, type: dbStatus.connectionType } }; 
    ...
    app.get('/', (req, res) => { 
      res.status(200).json({ message: 'TitanBot System Online', version: '2.0.0', timestamp: new Date().toISOString() }); 
    }); 
...
bot.on("messageCreate", async (message) => { 
  ...
  const allowedChannels = [ '1493324135785562222', '1493324237249970176' ]; 
  if (allowedChannels.includes(message.channel.id)) { 
    ...
    // تم تعديل رابط الصورة هنا إلى الرابط المباشر الدائم لكي تظهر كاملة بدون فواصل
    await message.channel.send({ content: "https://i.ibb.co/C2fhyL6/1000290040.jpg" }).catch(() => {}); 
  } 
...
bot.on('interactionCreate', async interaction => { 
  ...
  if (interaction.customId === 'rules') { 
    await interaction.reply({ ephemeral: true, embeds: [{ color: 0x8B0000, title: '📜 قوانين السيرفر', description: `# قوانين TOKYO COMMUNITY\n* ممنوع السب الا في حالة المزاح\n* يُمنع منعا باتاً التحرش بجميع أنواعه\n* ممنوع ذكر الشواذ\n* ممنوع التحدث في الدين\n* ممنوع الترويج بكل أشكاله\n* ممنوع نشر/ارسال اي شيء إباحي/جنسي\n* ممنوع العنصرية إلا في حالة المزاح\n* ممنوع الاهانة\n* ممنوع السبام\n* ممنوع إزعاج اي شخص بالمنشن أو غيره` }] }); 
  } 
  if (interaction.customId === 'about') { 
    await interaction.reply({ ephemeral: true, embeds: [{ color: 0x8B0000, title: '🌸 من نحن', description: `**سيرفر طوكيو !** هو سيرفر عربي مميز يجمع الاولاد و البنات ! 🌸 نوفر لك: • بيئة آمنة ومريحة • أعضاء رائعين ومحترمين • محتوى انمي والعاب انضم لعائلتنا وكن جزءاً من المجتمع! ✨` }] }); 
  } 
  if (interaction.customId === 'boost') { 
    await interaction.reply({ ephemeral: true, embeds: [{ color: 0x8B0000, title: '💎 مميزات البوست', description: `• رول خاص ولون مميز\n• 5 لفلاتف إضافية\n• صلاحيات إضافية\n• وعندما تضع البوست تحصل على رتبة <@&1505179614828302446>` }] }); 
  } 
  if (interaction.customId === 'roles') { 
    await interaction.reply({ ephemeral: true, embeds: [{ color: 0x8B0000, title: '🎌 الرتب الخاصة', description: `• رول خاص ولون مميز\n• 5 لفلاتف إضافية\n• صلاحيات إضافية` }] }); 
  } 
}); 
