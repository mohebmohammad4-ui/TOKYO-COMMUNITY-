...
// دالة مساعدة لجلب إعدادات السيرفر من قاعدة البيانات في الوقت الفعلي
async function getDynamicSettings(guildId) {
  if (!guildId || mongoose.connection.readyState !== 1) return null;
  try {
    const Guild = mongoose.models.GuildConfig;
    if (Guild) {
      return await Guild.findOne({ guildId });
    }
  } catch (err) {
    console.error("خطأ أثناء جلب الإعدادات الحركية لـ bot.js:", err);
  }
  return null;
}
...
export const botConfig = {
  ...
  counters: {
    ...
    types: {
      ...
      bots: {
        ...
        getCount: (guild) => guild.members.cache.filter((m) => m.user.bot).size.toString() || "0",
      },
      members_only: {
        name: "👤 Humans",
  ...
  // =========================
  // GENERIC BOT MESSAGES (تم ربطها ديناميكياً للتحقق من الحظر)
  // =========================
  messages: {
  ...
  // =========================
  // FEATURE TOGGLES (تتغير تلقائياً حسب تفعيل نظام الـ XP في قاعدة البيانات)
  // =========================
  features: {
    economy: true,
    leveling: true, // مفعّل لتمرير البيانات للـ Handler دائماً
    moderation: true,
    logging: true,
    welcome: true,
    tickets: true,  
...
// =========================================================================
// دالات التحقق وفحص البيانات المساعدة (تُترك كما هي لضمان استقرار البوت عند الإقلاع)
// =========================================================================
